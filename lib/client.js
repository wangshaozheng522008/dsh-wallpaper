 ...
 *  - uploads the picked image to the host and persists `{image, opacity, accent}`
 *    through the plugin's own state routes (the web settings RPC only serves an
 *    explicit namespace allow-list, so this plugin owns its state surface),
 ...
 * services (slots, locale, remote, theme), never through imports.
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var STATE_ENDPOINT = "/api/dsh-wallpaper/state";
		...
		var WallpaperService = class {
			constructor(ctx) {
				...
				// Initial state and cross-tab / host-side invalidation.
				this.fetchState().catch(() => {});
				ctx.effect(() => ctx.remote.$on("settings/document-updated", (ns) => {
					if (ns !== WALLPAPER_NAMESPACE) return;
					this.fetchState().catch(() => {});
				}), "dsh-wallpaper: settings document invalidation");
			...
			/** GET /api/dsh-wallpaper/state and adopt it. */
			async fetchState() {
				var response = await fetch(STATE_ENDPOINT);
				if (!response.ok) throw new Error(`HTTP ${response.status}`);
				var result = await response.json();
				var value = result && typeof result.value === "object" && result.value !== null ? result.value : {};
				this.adoptValue(value);
			}

			/** POST /api/dsh-wallpaper/state (wholesale replace of the persisted section). */
			async writeState(section) {
				var response = await fetch(STATE_ENDPOINT, {
					method: "POST",
			...
			/** Normalize an incoming persisted value and apply it when changed. */
			adoptValue(value) {
			...
			async upload(file) {
				...
				try {
					...
					await this.writeState({ image: result.url, opacity: DEFAULT_OPACITY, accent });
					this.adoptValue({ image: result.url, opacity: DEFAULT_OPACITY, accent });
				} catch (error) {
			...
			setOpacity(value) {
				...
				this.writeState({ image: this.state.image, opacity: next, accent: this.state.accent }).catch(() => {});
				this.adoptValue({ image: this.state.image, opacity: next, accent: this.state.accent });
			}
